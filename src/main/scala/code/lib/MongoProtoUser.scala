...
trait MetaMegaProtoUser[ModelType <: MegaProtoUser[ModelType]] extends MongoMetaRecord[ModelType] with ObjectIdPk[ModelType] with GenProtoUser {
  self: ModelType =>
   ...
   */
    def getUniqueId(): String = in.id.is.toString
